...
- `ck-pi-zen-session 0.1.12`：全局 Fetch 守卫与会话压缩（Compaction/Summarization）403 根治防御。拦截 Pi 绕过 Agent 生命周期直接发起的纯文本摘要请求，自动注入官方 6 大核心工具链、毫秒级降序 Session ID 与升序 Request ID，对齐 `stream_options`，彻底根除 `403 FreeTierError`。
